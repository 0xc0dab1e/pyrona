...
def main(visualize):
    ...
    # buffer for agents x and y coordinates
    iXY = np.zeros((len(agents),), dtype=[('idx', int),
                                          ('x', float),
                                          ('y', float)])
    ...
    # prepare timestamp prefix for additional files distinction
    timestamp = datetime.now().strftime("%H:%M:%S")
    tag = args.name +'_'+  timestamp
    
    print("Genetrating output files with the tag: " + tag)
    
    # store the config file for the reference
    dump_config_path = os.path.join(
        paths["configs"], "config_"+ tag +".yaml")
    
    shutil.copy(config_fn, dump_config_path)
    
    # create intermediate file with agent meetings
    ...
    with open(meets_table_path, 'wb') as file:
        ...
        for eval_time in tqdm(eval_times):
            ...
            """
            meets_curr = detect_meetings(agents, iXY, eval_time,
                                         config, visualize)
